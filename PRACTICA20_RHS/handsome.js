...
function initializeHandsontable(dataCitas, metadata) {
    ...
    items: {
        ...
        // Agregamos el 4to parámetro: 'source'
beforeRemoveRow: function (index, amount, physicalRows, source) {
    
    // 1. EL PORTERO: Si es automático, déjalo pasar sin preguntar
    // 'SOCKET_UPDATE' = Viene de la otra pantalla
    // 'LOCAL_DELETE' = Viene de tu propio Ajax exitoso
    if (source === 'SOCKET_UPDATE' || source === 'LOCAL_DELETE') {
        return true; 
    }

    // 2. LÓGICA MANUAL (Solo si TÚ le das clic derecho -> Eliminar)
    const idsToDelete = [];
    physicalRows.forEach(rowIndex => {
        // Nota: Asegúrate de usar 'getSourceDataAtRow' correctamente con el índice físico
        // A veces physicalRows trae índices visuales dependiendo de la versión,
        // pero asumiremos que tu lógica de obtención de ID ya te funciona.
    ...
    if (idsToDelete.length > 0) {
        ...
        // TRUCO PRO: Devolvemos FALSE aquí para que la fila NO se borre todavía.
        // Esperamos a que 'ajaxEliminarCita' tenga éxito y ella misma borrará
        // la fila usando 'LOCAL_DELETE', lo cual caerá en el paso 1.
        return false; 
    }
},
    ...
    // Bloqueamos todo lo que no sea interacción directa del usuario
    // Agregamos 'SOCKET_UPDATE'
    if (source === 'loadData' || source === 'id_population' || source === 'SOCKET_UPDATE' || !changes) {
        return; 
    }

    // 2. Si pasamos el filtro, procesamos la lógica de negocio
    changes.forEach(([row, prop, oldValue, newValue]) => {
        if (oldValue === newValue) return;
        
        // --- Lógica de autocompletado de teléfono ---
        if (prop === 'nom_eje_visual') {
            const ejecutivoEncontrado = listaEjecutivosGlobal.find(e => e.label === newValue);
            
            if (ejecutivoEncontrado) {
                // OJO: Aquí usamos 'auto_relleno', asegúrate de que esto no dispare un bucle si tienes lógica ligada a ello.
                // Como 'auto_relleno' no es 'loadData' ni 'SOCKET_UPDATE', pasará el filtro de arriba en la siguiente vuelta.
                // Asegúrate de agregar 'auto_relleno' al filtro de arriba SI NO quieres que se guarde en BD (pero usualmente sí quieres).
        ...
        if (rowData.id_cit) {
            ...
            // Evitamos guardar si solo estamos editando columnas visuales o vacías
...
function prepararDatosParaEnvio(rowData) {
    ...
    // 1. PRIMER INTENTO: Buscar por el nombre del dropdown
    ...
    if (!idEjecutivoFinal) {
        // Solo si estamos viendo a un ejecutivo (no un plantel)
        if (ejecutivoSeleccionadoID && String(ejecutivoSeleccionadoID).indexOf('P') !== 0) {
            idEjecutivoFinal = ejecutivoSeleccionadoID;
        }
    }
...
function ajaxEliminarCita(id) {
    ...
    $.ajax({
        ...
        success: function(response) { 
            ...
            // --- 1. ELIMINACIÓN VISUAL INMEDIATA (Esto es lo que te faltaba) ---
            // Buscamos en qué fila visual está ese ID
            const rowIndex = buscarFilaPorID(id);
            
            if (rowIndex !== -1) {
                // Quitamos la fila de TU pantalla al instante
                // Usamos 'LOCAL_DELETE' para identificar que fuimos nosotros
                hotInstance.alter('remove_row', rowIndex, 1, 'LOCAL_DELETE');
            }

            // --- 2. AVISAR AL SOCKET ---
            // Solo avisamos si la base de datos respondió bien
...
function ajaxActualizarCita(rowData, prop, newValue) {
    ...
    // 1. Limpieza de datos (esto sigue igual para PHP)
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            // 2. EMITIR EL CAMBIO REAL AL SOCKET
            emitirCambio('UPDATE', {
                id_cit: datosLimpios.id_cit, // El ID de la cita
                campo: prop,                 // EJ: 'nom_cit' (La columna real)
                valor: newValue              // EJ: 'Pedro' (El valor real)
            });
...
function ajaxGuardarCita(rowData, visualRowIndex, prop, value) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (nuevoID > 0) {
                // 1. Actualizamos el ID oculto en la tabla local
                ...
                // 2. AVISAR AL SOCKET 
                // Usamos 'UPDATE' porque para los otros usuarios la fila ya existe (está vacía),
                // solo necesitamos que se llene el dato.
                emitirCambio('CREATE', {
                    datos_fila: datosLimpios // Enviamos el objeto completo {id, nombre, fecha...}
                });
...
function procesarMensajeSocket(mensaje) {
    console.log("📩 Socket Event:", mensaje.tipo);
    if (!hotInstance) return;
    ...
    if (mensaje.tipo === 'UPDATE') {
        ...
        // A. Actualizar la tabla (SOLO SI LA VEO)
        if (rowIndex !== -1) {
            hotInstance.setDataAtRowProp(rowIndex, datos.campo, datos.valor, 'SOCKET_UPDATE');
            
            // Efecto visual en la celda
            const colIndex = hotInstance.propToCol(datos.campo);
            hotInstance.setCellMeta(rowIndex, colIndex, 'className', 'celda-actualizada');
            hotInstance.render();
        }

        // B. Notificación (SIEMPRE, aunque no vea la fila) 
        // Esta línea ahora está FUERA del if(rowIndex !== -1)
        mostrarToast(`Cambio en cita #${datos.id_cit}: ${datos.valor}`);
    }

    // --- CASO 2: BAJA (DELETE) ---
    else if (mensaje.tipo === 'DELETE') {
        const idParaBorrar = mensaje.datos.id_cit;
        console.log(" Intento de borrado remoto. ID:", idParaBorrar);
        const rowIndex = buscarFilaPorID(idParaBorrar);
        console.log("Resultado búsqueda índice:", rowIndex);
        // A. Borrar de la tabla (SOLO SI LA VEO)
        if (rowIndex !== -1) {
            hotInstance.alter('remove_row', rowIndex, 1, 'SOCKET_UPDATE');
            console.log("Fila eliminada visualmente en índice:", rowIndex);
        } else {
            console.warn("No se encontró la fila en esta pantalla (¿Quizás filtro activo?)");
        }

        // B. Notificación (SIEMPRE) 
        mostrarToast(`Cita #${datos.id_cit} eliminada remotamente`);
    }
 ...
 // --- CASO 3: ALTA (CREATE) ---
        ...
        const correspondeAmiVista = (String(filaDatos.id_eje2) === String(ejecutivoSeleccionadoID));

        // 2. INSERCIÓN EN TABLA (Solo si no existe Y corresponde a mi vista)
        if (existe === -1 && correspondeAmiVista) {
            
            // A. Insertar fila vacía arriba
        ...
        // Si NO corresponde a mi vista, solo logueamos en consola (Opcional)
        else if (!correspondeAmiVista) {
            console.log(" Cita recibida pero ignorada visualmente (es de otro ejecutivo).");
        }

        // 3. NOTIFICACIÓN (SIEMPRE)    
        // Al estar fuera de los IFs de arriba, se ejecuta siempre.
        // Como querías el mismo mensaje en ambos casos, borramos la condición extra.
        mostrarToast(`Nueva cita #${filaDatos.id_cit} creada: ${filaDatos.nom_cit}`);
    }
}

// Auxiliar para encontrar la fila visual basada en el ID oculto
// Agrega esto en handsome.js
