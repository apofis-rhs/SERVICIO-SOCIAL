...
// ✨ CAMBIO: Si no viene id_plantel, pero sí sabemos el ejecutivo, heredamos su plantel
if (empty($_POST['id_plantel']) && !empty($_POST['id_eje2'])) {
    $idEjeAsignado = (int)$_POST['id_eje2'];
    $resPla = $connection->query("SELECT id_pla1 FROM ejecutivo WHERE id_eje = $idEjeAsignado");
    if ($resPla && $rowPla = $resPla->fetch_assoc()) {
        $_POST['id_plantel'] = $rowPla['id_pla1'];
    }
}
