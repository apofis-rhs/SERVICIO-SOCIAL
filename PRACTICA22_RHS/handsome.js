...
function initializeHandsontable(dataCitas, metadata, comentariosIniciales = [], estilosIniciales = []) {
    ...
    const columns = metadata.map(col => {
        ...
        // 🎨 RENDERIZADOR MAESTRO (Aquí ocurre la magia visual)
        // Obtenemos el renderizador base según el tipo de columna (texto, fecha, etc.)
        ...
        colDef.renderer = function(instance, td, row, col, prop, value, cellProperties) {
            // A. Dibujamos el contenido normal (texto, fecha, dropdown)
            baseRenderer.apply(this, arguments);

            // B. Lógica de Rango Fijo (Tu estilo original)
            if (prop === 'rango_fijo') {
                td.style.fontWeight = 'bold';
                td.style.color = '#c92f2fff';
                td.style.backgroundColor = '#f0f0f0';
                td.style.borderRight = '2px solid #ccc';
                return; // No permitimos pintar encima del rango fijo
            }

            // C. 🎨 APLICAR COLOR PERSONALIZADO (Si existe en el mapa)
    ...
    hotInstance = new Handsontable(container, {
        ...
        afterChange: function (changes, source) {
            if (source === 'loadData' || source === 'id_population' || source === 'SOCKET_UPDATE' || !changes) return; 
...
function procesarMensajeSocket(mensaje) {
 ...
 // --- CASO 3: ALTA (CREATE) ---
    ...
    //  CASO 4: RECIBIR COMENTARIO DE OTRO USUARIO
        ...
        // Solo actuamos si la fila es visible en mi pantalla
        if (rowIndex !== -1 && hotInstance) {
            
            // IMPORTANTE: Activamos bandera
            isAutosaving = true; 
            
            const colIndex = hotInstance.propToCol(d.campo);
            const pluginComments = hotInstance.getPlugin('comments');
            
            // 1. Aplicar o borrar el triangulito
            if (d.comentario === '') {
            ...
            // --- 2. CORRECCIÓN DE ALINEACIÓN Y FLASH ---
            
            // A. Guardamos la clase que ya tenía la celda (ej: 'htCenter', 'htRight', etc.)
            const cellMeta = hotInstance.getCellMeta(rowIndex, colIndex);
            const claseOriginal = cellMeta.className || '';

            // B. Aplicamos: Clase Original + Espacio + Clase Flash
            // Usamos trim() para evitar espacios extra si no había clase original
            const nuevaClase = (claseOriginal + ' celda-comentario-flash').trim();
            
            hotInstance.setCellMeta(rowIndex, colIndex, 'className', nuevaClase);
            hotInstance.render();

            // C. Quitamos el color después de 2 segundos, pero RESTAURAMOS la alineación
            setTimeout(() => {
                // Volvemos a poner EXACTAMENTE lo que había antes (ej: 'htCenter')
                hotInstance.setCellMeta(rowIndex, colIndex, 'className', claseOriginal); 
                hotInstance.render();
            }, 2000);

            // Desactivamos bandera inmediatamente para permitir seguir trabajando
            isAutosaving = false;
        }
    }

    
    //  CASO NUEVO: ACTUALIZACIÓN DE COLOR
    else if (mensaje.tipo === 'COLOR_UPDATE') {
        const d = mensaje.datos;
        const rowIndex = buscarFilaPorID(d.id_cit);
        
        console.log("Recibido cambio de color:", d);

        // 1. Actualizar mapa de colores en memoria
        const key = `${d.id_cit}_${d.campo}`;
        
        if (d.color === 'reset') {
            delete mapaColores[key]; // Borramos si es reset
        } else {
            mapaColores[key] = d.color; // Guardamos el nuevo color
        }

        mostrarToast(`🎨 Estilo actualizado en cita #${d.id_cit}`);

        // 2. Si la fila es visible, repintar
        if (rowIndex !== -1 && hotInstance) {
            // Renderizamos para que Handsontable lea el nuevo mapaColores
            hotInstance.render();
            ...
            if (td) {
                // Guardamos el color final
                const colorFinal = d.color === 'reset' ? '' : d.color;
                
                // Hacemos un flash blanco para avisar que cambió
                td.style.transition = "background-color 0.2s";
                td.style.backgroundColor = "#ffffff"; // Flash blanco brillante
                
                setTimeout(() => {
                    // Volvemos suavemente al color definitivo (rojo, verde, etc.)
                    td.style.transition = "background-color 0.8s";
                    // Si es reset, dejamos que el CSS base tome el control (string vacío)
                    td.style.backgroundColor = colorFinal; 
                }, 200);
            }
