...
function initializeHandsontable(dataCitas, metadata, comentariosIniciales = [], estilosIniciales = []) {
    ...
    const columns = metadata.map(col => {
        ...
        colDef.renderer = function(instance, td, row, col, prop, value, cellProperties) {
            ...
            // B. 🟧 Lógica de COLOR para ESTATUS (¡ESTO ES LO QUE FALTABA!)
            if (prop === 'est_cit') {
...
function procesarMensajeSocket(mensaje) {
    ...
    if (mensaje.tipo === 'UPDATE') {
        console.log("📩 Socket Recibido UPDATE:", datos); 

        const rowIndex = buscarFilaPorID(datos.id_cit);
        
        // A. Actualizar la tabla (SOLO SI LA VEO EN PANTALLA)
        if (rowIndex !== -1) {
            // 1. Actualizar el dato visualmente
            hotInstance.setDataAtRowProp(rowIndex, datos.campo, datos.valor, 'SOCKET_UPDATE');
            
            // 2. Gestión de Colores
            if (datos.campo === 'est_cit') {
                hotInstance.render(); // Forzar repintado para el nuevo color
                
                // Efecto Flash Blanco
                const colIndex = hotInstance.propToCol('est_cit');
                ...
                // Efecto borde verde para otros campos
        ...
        setTimeout(() => {
            if (typeof actualizarContadores === 'function') {
                const dataFresca = hotInstance.getSourceData();
                actualizarContadores(dataFresca);
                console.log("📊 Contadores actualizados remotamente");
            }
        }, 50);
    ...
    // =============================================
        ...
        let idEjecutivoVisto = ejecutivoSeleccionadoID; 
        
        // Solo agregamos si corresponde a la vista actual
        if (String(filaDatos.id_eje2) === String(idEjecutivoVisto)) {
             ...
             if (existe === -1) {
                ...
                // Aseguramos que el estatus llegue, o ponemos el default
        ...
        if (typeof mostrarToast === "function") {
            mostrarToast(`Nueva cita #${filaDatos.id_cit} creada`);
        }
    }
...
// --- FUNCIÓN PARA ACTUALIZAR BARRA DE CONTADORES ---
function actualizarContadores(datosTabla) {
    ...
    datosTabla.forEach(fila => {
        ...
        if (fila.id_cit) {
            // Si no tiene estatus, asumimos el default
    ...
    // 3. Dibujar etiquetas (Solo las que tengan > 0)
    // Usamos el catálogo global para saber qué colores usar
    Object.keys(CATALOGO_ESTATUS).forEach(key => {
        if (conteos[key]) { 
            const colorFondo = CATALOGO_ESTATUS[key];
            const cantidad = conteos[key];
            
            // Crear el elemento visual (Badge)
            const badge = document.createElement('div');
            badge.className = 'badge-contador'; // Clase para estilos CSS (opcional)
            badge.style.cssText = `
            ...
            // Contenido: Nombre + Círculo blanco con el número
            badge.innerHTML = `
                ${key} 
                <span style="background: white; padding: 0px 6px; border-radius: 10px; font-size: 10px;">${cantidad}</span>
            `;
