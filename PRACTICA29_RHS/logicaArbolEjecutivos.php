...
if ($accion == 'crear_o_renombrar') {
    ...
    // B) ES EXISTENTE (UPDATE NOMBRE) 
        ...
        // 1. Validar que el texto no venga vacío
        if (trim($texto) === '') {
             echo json_encode(array("error" => "El nombre no puede estar vacío"));
             exit;
        }

        // 2. Obtener nombre anterior (para el historial)
        $nombreViejo = '';
        ...
        // 3. Ejecutar UPDATE (Sin condicionales, actualizamos directo)
        $sql = "UPDATE ejecutivo SET nom_eje = '$texto' WHERE id_eje = $idReal";
        
        if ($connection->query($sql)) {
            
            // 4. Guardar Historial (Solo si son diferentes)
            if ($nombreViejo !== $texto && $nombreViejo !== '') {
                $desc = "El usuario $ejecutivoLog renombró a '$nombreViejo' como '$texto'.";
                registrarHistorialEjecutivo($connection, $ejecutivoLog, 'cambio', $desc, $idReal);
            }

            // ✅ ESTO ES LO QUE TE FALTABA: 
            // Devolver el ID explícitamente y el nuevo texto para que JS lo sepa
