...
function initializeHandsontable(dataCitas, metadata, comentariosIniciales = []) {
    isLoading = true; 
    ...
    hotInstance = new Handsontable(container, {
            ...
            items: {
                "commentsAddEdit": { 
                    ...
                    hidden: function() {
                        ...
                        // Si no hay datos o no hay ID de cita, ocultamos la opción
                        return !rowData || !rowData.id_cit; 
                    }
                },
                ...
                "ver_historial": {
                    ...
                    hidden: function() { // Opcional: También ocultar historial si está vacía
                        const selection = this.getSelectedLast();
                        const row = selection[0];
                        const rowData = this.getSourceDataAtRow(row);
                        return !rowData || !rowData.id_cit;
        ...
        afterSetCellMeta: function (row, col, key, value) {
            // No enviar al socket si estamos cargando o si viene del socket
    ...
    // 3. Carga de Comentarios (Persistencia)
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response && response.data && response.metadata) {
                ...
                initializeHandsontable(response.data, response.metadata, response.comentarios || []);
            }
        },
