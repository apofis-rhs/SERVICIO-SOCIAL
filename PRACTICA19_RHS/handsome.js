    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (nuevoID > 0) {
                ...
                // Actualizamos el objeto local para futuros updates
                rowData.id_cit = nuevoID;

                // 2. AVISAR AL SOCKET (Corregido) 📡
                // Usamos 'UPDATE' porque para los otros usuarios la fila ya existe (está vacía),
                // solo necesitamos que se llene el dato.
                emitirCambio('UPDATE', {
                    id_cit: nuevoID,   // ¡USAMOS EL ID RECIÉN CREADO!
                    campo: prop,       // La columna que editaste (ej: 'nom_cit')
                    valor: value       // El valor que escribiste (ej: 'Juan Perez')
                });
...
function procesarMensajeSocket(mensaje) {
    ...
    // Supongamos que mensaje.datos trae { id_cit: 125, campo: 'nom_cit', valor: 'Nuevo Nombre' }
    const datos = mensaje.datos;

    if (mensaje.tipo === 'UPDATE') {
        // 1. Buscar la fila en la tabla que tenga ese ID
        // Handsontable no busca por ID nativamente fácil, hay que iterar o usar índices físicos
        const rowIndex = buscarFilaPorID(datos.id_cit); 

        if (rowIndex !== -1) {
            // 2. Actualizar el dato SIN disparar el bucle infinito
            // Usamos el 4to parámetro 'source' como bandera
            hotInstance.setDataAtRowProp(rowIndex, datos.campo, datos.valor, 'SOCKET_UPDATE');

            // 3. Feedback Visual (Celda Verde)
            aplicarEstiloCelda(rowIndex, datos.campo);
            
            // 4. Badge
            mostrarToast(`Actualización externa en cita #${datos.id_cit}`);
        }
    }
}
...
function buscarFilaPorID(idBuscado) {
    const datos = hotInstance.getSourceData();
    return datos.findIndex(row => row.id_cit == idBuscado);
}
