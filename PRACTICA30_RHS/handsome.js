...
$(document).ready(function () {
    ...
    $('#filtro_tipo_eje').on('change', function() {
        if (ejecutivoSeleccionadoID) {
            cargarCitas(); // Recarga la tabla con el nuevo filtro
        }
        if (typeof actualizarBadgesArbol === 'function') {
            actualizarBadgesArbol(); // Recarga el arbolito 
        }
    });
    //  ESCUCHAR CAMBIO EN EL FILTRO DE PAPELERA 
    $('#filtro_estado_cita').on('change', function() {
        if (ejecutivoSeleccionadoID) {
            cargarCitas(); // Recarga la tabla para mostrar activas o borradas
        }
    });
});
...
function initializeHandsontable(dataCitas, metadata, comentariosIniciales = [], estilosIniciales = []) {
    ...
    hotInstance = new Handsontable(container, {
        ...
        search: true,
        comments: true, 
        //PRACTICA 28 - BLOQUEO DE EDICIÓN Y SELECCIÓN MASIVA

        selectionMode: 'single', // Solo permite seleccionar de a una celda por vez 
        fillHandle: false,       // Desactiva el cuadrito de arrastre 
        outsideClickDeselects: true, // Inhabilita/deselecciona al dar clic fuera de la tabla
        ...
        contextMenu: {
            items: {
                ...
                "remove_row": { 
                    name: '❌ Eliminar cita',
                    hidden: function() {
                        return $('#filtro_estado_cita').val() === '0'; // Se oculta si estamos en Papelera
                    }
                },
                // ✨ NUEVO: El botón de Restaurar (Solo sale en la papelera)
                "restaurar_cita": {
                    name: '♻️ Restaurar Cita',
                    hidden: function() {
                        return $('#filtro_estado_cita').val() !== '0'; // Se oculta si estamos en Activas
                    },
                    callback: function(key, selection) {
                        const row = selection[0].start.row;
                        const rowData = this.getSourceDataAtRow(row);
                        if (rowData && rowData.id_cit) {
                            if (confirm(`¿Estás seguro de restaurar la cita #${rowData.id_cit}?`)) {
                                ajaxRestaurarCita(rowData.id_cit, row, rowData);
                            }
                        } else {
                            alert("Fila vacía, no hay nada que restaurar.");
                        }
                    }
                },
                "hsep2": "---------",
        ...
        beforeRemoveRow: function (index, amount, physicalRows, source) {
            if (source === 'SOCKET_UPDATE' || source === 'LOCAL_DELETE' || source === 'LOCAL_RESTORE') return true;
            const idsToDelete = [];
...
function cargarCitas(modoForzado = null) {
    ...
    var tipoEje = $('#filtro_tipo_eje').val() || 'Todos';

    var estadoCita = $('#filtro_estado_cita').val() || '1'; 

    console.log(" Cargando citas con Modo:", modo, "Área:", tipoEje, "Estado:", estadoCita);
...
//  NUEVO: Función para restaurar una cita desde la papelera. Recibe el ID de la cita y el índice de la fila para eliminarla visualmente.
// ✨ VERSIÓN BLINDADA: Ahora asegura que todos los datos viajen ✨
function ajaxRestaurarCita(idCita, rowIndex, rowData) {
    console.log("Preparando resurrección para ID:", idCita, "Datos fila:", rowData);

    // Salvavidas: Asegurar que el plantel viaje en la mochila para que la otra pantalla lo deje entrar
    if (!rowData.id_plantel && String(ejecutivoSeleccionadoID).startsWith('P')) {
        rowData.id_plantel = String(ejecutivoSeleccionadoID).replace('P', '');
    }

    $.ajax({
        url: 'logicaRestaurarCit.php', 
        type: 'POST',
        data: { id_cit: idCita },
        success: function(response) { 
            console.log("PHP confirmó restauración:", response);

            // 1. Borramos la fila VISUALMENTE de la papelera local
            hotInstance.alter('remove_row', rowIndex, 1, 'LOCAL_RESTORE');

            // 2. Avisamos por el altavoz (WebSocket)
            if (typeof emitirCambio === "function") {
                console.log("📡 Emitiendo socket RESTORE al servidor...");
                emitirCambio('RESTORE', { 
                    id_cit: idCita, 
                    datos_fila: rowData 
                });
            }
            
            mostrarToast(`Cita #${idCita} restaurada con éxito ♻️`);
            actualizarBadgesArbol();
        },
        error: function(xhr) { 
            console.error("Error al restaurar:", xhr.responseText); 
            alert("Error al restaurar la cita. Verifica la consola.");
        }
    });
}
...
function procesarMensajeSocket(mensaje) {
    ...
    if (mensaje.tipo === 'UPDATE') {
        ...
        if (rowIndex !== -1) {
            ...
            if (typeof mostrarToast === "function") {
                mostrarToast(`Cita #${datos.id_cit} actualizada remotamente`);
            }
        } 
    ...
    // =============================================
    // CASO 6: RESTAURAR (RESTORE)
    // =============================================
    else if (mensaje.tipo === 'RESTORE') {
        const d = mensaje.datos;
        const estadoActual = $('#filtro_estado_cita').val(); // '1' (Activas) o '0' (Papelera)
        
        if (estadoActual === '0') {
            // Si el compañero está viendo la Papelera, la cita resucitada debe desaparecer de ahí
            const rowIndex = buscarFilaPorID(d.id_cit);
            if (rowIndex !== -1) {
                hotInstance.alter('remove_row', rowIndex, 1, 'SOCKET_UPDATE');
                mostrarToast(`La cita #${d.id_cit} fue restaurada por otro usuario ♻️`);
            }
        } else {
            // Le pasamos el paquete al bloque CREATE para que use toda su inteligencia de filtros
            procesarMensajeSocket({
                tipo: 'CREATE',
                datos: { 
                    datos_fila: d.datos_fila,
                    emisor_id: 'servidor' // Evitamos que el filtro de seguridad lo bloquee
                }
            });
        }
        
        // Actualizamos bolitas para todos
        actualizarBadgesArbol();
    }
}
...
function actualizarBadgesArbol() {
    ...
    timerBadgesArbol = setTimeout(() => {
        ...
        if (arbol) {
            console.log(" Recargando arbolito y contadores...");
            // refresh() recarga el JSON desde PHP sin cerrar las carpetas abiertas
            arbol.refresh(true,true); 
        }
    }, 500); 
}