...
if (isset($_GET['tipo_eje']) && $_GET['tipo_eje'] !== 'Todos' && $_GET['tipo_eje'] !== '') {
    ...
    $filtroTipoEjeArbol = " AND e.tipo_eje = '$tipoEje' ";
}
...
$sqlEje = "SELECT 
           ...
           FROM ejecutivo e 
           WHERE e.eli_eje = 1 $filtroTipoEjeArbol";
...
// ✨ NUEVO: Salvavidas por si filtramos al jefe directo, el nodo se cuelga del Plantel para no desaparecer
    $padreFinal = ($row['id_padre'] != NULL && isset($todosEjecutivos[$row['id_padre']])) ? $row['id_padre'] : "P" . $row['id_pla1'];
    $data[] = array(
        "id" => $row['id_eje'],
        "parent" => $padreFinal,
        "text" => $textoNodo,
