...
function ajaxGuardarCita(rowData, visualRowIndex, prop, value) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            //  NUEVO: ABRIMOS EL PAQUETE JSON  
            let nuevoID = 0;
            let idPlantelCita = null;

            try {
                // Revisamos si jQuery ya lo convirtió en objeto o si sigue como texto
                let res = typeof response === 'string' ? JSON.parse(response) : response;
                nuevoID = parseInt(res.id_cit) || 0;
                idPlantelCita = res.id_plantel || null;
            } catch (e) {
                // Por si hay algún error raro, intentamos leerlo a la antigüita
                nuevoID = parseInt(response) || 0;
            }
            //  FIN DE LO NUEVO 
            ...
            if (nuevoID > 0) {
                ...
                //  NUEVO: Le inyectamos el plantel a los datos que van a viajar por el Socket 
                datosLimpios.id_plantel = idPlantelCita;

                // 2. AVISAR AL SOCKET 
                emitirCambio('CREATE', {
                    datos_fila: datosLimpios // Enviamos el objeto completo con el plantel incluido
                });
